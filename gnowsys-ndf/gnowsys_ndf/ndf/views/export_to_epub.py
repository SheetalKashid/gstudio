...
oebps_files = ["Fonts", "Audio", "Images", "Video", "Text", "Styles", "Misc"]
oebps_path = None
tool_mapping = {}
# global epub_root
# global epub_name
...
def copy_file_and_update_content_file(file_node, source_ele, src_val, epub_name):
    ...
    """for each_src_ele in source_ele:
        print(each_src_ele)"""
...
def parse_content(path, content_soup, epub_name):
    ...
    for index,each_src in enumerate(all_src,start=1):
        print(index)
        print("*")*60
        ...
        if src_attr.startswith("/media"): # file
            ...
            #file_nn=src_attr.split("/")[-1]
            #print(file_nn)
            #file_node["src_attr"] = "../Audio/"+ file_name
            #print(file_node)
            #print("=")*70
...
def build_html(path,obj, epub_name):
    ...
    for each_obj in obj.values():
        ...
        #the above line is to display the generated xhtml code based on the given object value.
...
def fill_from_static():
    ...
    #trial = ['/static/ndf/css/clix-activity-styles.css', '/static/ndf/css/rubik-fonts.css','/static/ndf/css/demo.css']
    ...
    # css_rf = open('/static/ndf/css/demo.css', 'r')
    # tmp_file = css_rf.readlines()
    # css_rf.close()

    # with open('/tmp/demo.css', 'w+') as tmp_css_file:
    #     for each_css_line in tmp_file:
    #         if '@import' not in each_css_line:
    #             tmp_css_file.write(each_css_line)
        ...
        for dep_type, dep_list in dependencies_data.items():
            ...
            for each_dep in dep_list:
                ...
                if each_dep in clean_css_files:
                    ...
                    #print("updated new file path "+new_filepath)
                else:
                    shutil.copyfile(each_dep, new_filepath)
                    #print(new_filepath)
                    #print("nnn file path"+new_filepath)
...
def create_epub(node_obj):
    epub_disp_name = None
    global epub_name
    epub_name = node_obj.name                      #name of the node is the epub_name
    if node_obj.altnames:
        epub_disp_name = node_obj.altnames
    else:
        epub_disp_name = node_obj.name                
    content_list = node_obj.collection_dict         #contentlist is a collection of activities in that particular lesson.
    #print(content_list)
    if not os.path.exists(GSTUDIO_EPUBS_LOC_PATH):
        os.makedirs(GSTUDIO_EPUBS_LOC_PATH)                    #makedirs= recursive directory creation function
    ...
    epub_name = slugify(epub_name + "_"+ str(datetimestamp))        #lessonname_datetimestamp
    #print(epub_name) 
    global epub_root     
    epub_root = os.path.join(GSTUDIO_EPUBS_LOC_PATH, epub_name)  #Join one or more path components, concatenation of path and any members of *paths with exactly one directory separator.
    #print(epub_root)
    #files=os.listdir(epub_root)
    #print(files)
    #filee=os.path.abspath(epub_root)
    #print(filee)
    #files=os.path.abspath(GSTUDIO_EPUBS_LOC_PATH)
    #print(files)
    #print(GSTUDIO_EPUBS_LOC_PATH)
